...
| Commit metadata | A personal email is present in all ten reachable commits; publication is paused by owner choice |
...
| --- | --- | --- |
| Release blocker | Public visibility would expose a personal address in commit metadata. | Repository remains private. A rewrite plan is in `reports/publication_plan.md`; no history rewrite or visibility change has been performed. |
...
- Kept the repository private when commit metadata exposure was discovered; publication now has a staged plan rather than an implicit visibility change.
